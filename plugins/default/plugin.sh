...
# DESCRIPTION:
# ============
# Default plugin is intended for testing the single-container application
...
plugin_print_help() {
  ...
  run IMAGE RESULTS_DIR -- Runs a new container which is based on the image
                           IMAGE (which contains the tests configuration and was
                           built by 'setup' command), creates a directory
                           RESULTS_DIR, copies all the tests configured for
                           the IMAGE into it and mounts it into the new
                           container. Logs from testing will also be placed into
                           the RESULTS_DIR directory.
...
plugin_setup() {
	...
	lock || return $?
	TEST_IMAGE_NAME="w_default/$image/$(date +%Y%m%d_%H%M%S)"
	sleep 1
	unlock || return $?
...
# container. Returns 0 on success, 1 on error.
plugin_run() {
	local section="" results_dir recipe cont cmd rv
	if [ $# -gt 2 ] || [ -z "$1" ] || [ -z "$2" ]; then
		eprintf "Usage: run IMAGE RESULTS_DIR\n"
		return 1
	fi
	eprintf "Plugin: $PLUGIN_DIR\n"
	...
	if [ "$recipe" == "-" ]; then
		eprintf "Error: Image is a base image and has no recipe: $1\n"
		return 1
	fi

	# Checks CMD for base images. If there is no CMD, uses "bin/bash",
	# otherwise uses the default CMD.
	cmd=$(docker inspect -f '{{ .ContainerConfig.Cmd }}' $1 \
		| sed 's/.*\(CMD.*"]\).*/\1/')
	if [[ $cmd != "CMD"*"["*"]" ]]; then
		cmd="/bin/bash"
	else
	...
	results_dir="$(cd $(dirname $2); pwd)/$(basename $2)"
	mkdir "$results_dir" || return $?

	# Gets locations of tests inside the image $1 from recipe section
	...
	# Runs the provided IMAGE.
	cont=$(docker run -dt -v /etc/localtime:/etc/localtime:ro \
		-l "$WHALE_LABEL" -l "$WHALE_IMG_LABEL=$1" \
		-v $results_dir:$TESTS_DIR:z $DOCKER_RUN_OPTIONS "$1" $cmd) \
		|| return $?
	cont="${cont:0:12}"
...
plugin_status() {
	local output output2 status test_stat supervisor_log cmd_exec
	if [ $# -gt 1 ] || [ -z "$1" ]; then
